...
class CirculationManager:
    ...
    def __init__(self) -> None:
        self.state_memory: dict[str, dict[str, Any]] = {
            "upstairs": {"start_time": None, "start_temp": None, "mode": None, "lockout_until": None},
            "downstairs": {"start_time": None, "start_temp": None, "mode": None, "lockout_until": None},
        }
    ...
    def load_learning_data(self, data: dict[str, Any]) -> None:
        ...
        for z_key in ["upstairs", "downstairs"]:
            if z_key in saved_outcomes:
                # Sanitize legacy outcomes: discard bogus success records where temp rose during cooling
                valid_recs = []
                for r in saved_outcomes[z_key]:
                    p = r.get("perf_vs_expected", 0.0)
                    if r.get("needs_cooling") and r.get("success") and p > 0.1:
                        continue
                    if (not r.get("needs_cooling")) and r.get("success") and p < -0.1:
                        continue
                    valid_recs.append(r)
                self.learning_outcomes[z_key] = valid_recs
    ...
    def predict_circulation_success(
        ...
        for record in history:
            if record.get("needs_cooling") != needs_cooling:
                continue
            p = record.get("perf_vs_expected", 0.0)
            if needs_cooling and record.get("success") and p > 0.1:
                continue
            if (not needs_cooling) and record.get("success") and p < -0.1:
                continue
    ...
    def evaluate_circulation(
        ...
        Returns: (action: 'turn_on' | 'turn_off' | 'hold', lockout_hours: float)
        """
        ...
        if is_active:
            ...
            active_mode = mem.get("mode") or ("cool" if needs_cooling else "heat")
            ...
            # 3. Stall detection (evaluated once min_runtime_minutes is reached)
            actual_change = area_temp - start_temp
            is_stalled = False
            perf_vs_expected = actual_change
            ...
            if model:
                ...
                if active_mode == "cool":
                    if actual_change > stall_margin or perf_vs_expected > -stall_margin:
                        is_stalled = True
                elif active_mode == "heat":
                    if actual_change < -stall_margin or perf_vs_expected < stall_margin:
                        is_stalled = True
            else:
                if active_mode == "cool" and actual_change > stall_margin:
                    is_stalled = True
                elif active_mode == "heat" and actual_change < -stall_margin:
                    is_stalled = True

            if is_stalled:
                self.record_outcome(
                    z_key=z_key,
                    needs_cooling=(active_mode == "cool"),
                    success=False,
                    perf_vs_expected=perf_vs_expected,
                    out_temp=out_temp,
                    area_temp=area_temp,
            ...
            # 4. Normal shutoff (Target comfort setpoint reached within deadband)
            if not needs_cooling and not needs_heating:
                ...
                return "turn_off", cooldown_hours
